...
# =========================
# Training config (INDIVIDUAL MARL — v2)
# =========================
SEED = 42
...
MODEL_DIR = "checkpoints_marl_v2"   # v2: separate folder, never overwrites v1
...
class IntersectionController:
    def __init__(self, tls_id, initialize_signals=True):
        ...
        self.last_action: int = 0      # typed as int — never None, safe for replay buffer
...
# =========================
# Episode runner (INDIVIDUAL)
# =========================
def run_episode(agents, train=True, explore=True):
    start_sumo()

    # Always initialise signals regardless of mode
    controllers = [
        IntersectionController(tls_id, initialize_signals=True)
        for tls_id in sorted(agents.keys())
    ]
    ...
    for step in range(MAX_STEPS):
        ...
        # Check termination before decision block so done flag is correct
        no_more_vehicles = (
        ...
        if step % DECISION_INTERVAL == 0:
            ...
            for ctrl in controllers:
                ...
                if ctrl.last_state is not None:
                    ...
                    agent.store(
                        ctrl.last_state,
                        ctrl.last_action,   # always int, never None
                        reward,
                        state,
                        done_flag,          # correct terminal signal
                    )
                    if train:
...
def main():
    ...
    for episode in range(1, MAX_EPISODES + 1):
        ...
        if current_wait < best_wait:
            ...
            print(f"💾 New best — avg wait: {best_wait:.2f}")
            for agent in agents.values():
                agent.save(tag="best")
    ...
    print(f"\n✅ Individual v2 training complete. Models and plots saved to '{MODEL_DIR}'.")
