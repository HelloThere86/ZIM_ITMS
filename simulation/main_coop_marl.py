...
# =========================
# Training config (COOPERATIVE MARL)
# =========================
SEED = 42
MAX_EPISODES = 200
MAX_STEPS = 3600

# STATE_DIM is now computed dynamically in bootstrap_agents() once we know
# how many intersections exist, so the net input always matches reality.
# Local state per agent = 9 features. Each agent also sees one neighbor = 9.
LOCAL_STATE_DIM = 9
ACTION_DIM = 2
...
MODEL_DIR = "checkpoints_coop_marl"
...
# =========================
class DQNAgent:
    def __init__(self, name, state_dim: int):
        self.name = name
    ...
    def select_action(self, state, explore=True) -> int:
    ...
    def store(self, state, action: int, reward: float, next_state, done: float):
...
class IntersectionController:
    def __init__(self, tls_id, initialize_signals=True):
        ...
        self.yellow_timer   = 0
        self.green_timer    = 0
        # FIX 5: typed as int, never None — safe to store in replay buffer
    ...
    def _extract_safe_states(self):
        ...
        for idx, phase in enumerate(phases):
            ...
            candidates.append({"idx": idx, "state": state, "ns_score": ns_score, "ew_score": ew_score})
        ...
        def next_yellow(phase_idx):
            ...
            return next_state if "y" in next_state.lower() else phases[phase_idx].state
    ...
    def request_action(self, action):
        if self.yellow_timer > 0 or action == self.current_action or self.green_timer < MIN_GREEN:
            return
    ...
    def get_local_state(self) -> np.ndarray:
        ...
        return np.array(
            [q_n/20, q_s/20, q_e/20, q_w/20,
             w_n/100, w_s/100, w_e/100, w_w/100,
    ...
    def get_local_score(self) -> float:
        return (
            sum(self._queue(v)   for v in self.approaches.values())
            + 0.05 * sum(self._waiting(v) for v in self.approaches.values())
        )
...
# =========================
# Cooperative state builder
# =========================
def build_coop_states(tls_ids, local_states: dict) -> dict:
    """
    FIX 3: Generalised neighbor sharing — works for any N >= 2.
    Each agent receives its own local state concatenated with its
    ring-neighbour's local state (same topology as before, but now
    correct regardless of N).  STATE_DIM must equal 2 * LOCAL_STATE_DIM.
    """
    coop = {}
    n = len(tls_ids)
    for i, tls_id in enumerate(tls_ids):
        neighbor_id   = tls_ids[(i + 1) % n]
        coop[tls_id]  = np.concatenate((local_states[tls_id], local_states[neighbor_id]))
    return coop
...
def bootstrap_agents():
    ...
    # FIX 3: state_dim computed from actual agent count, not a hardcoded constant
    state_dim = 2 * LOCAL_STATE_DIM   # own 9 + one neighbour's 9
    agents    = {tls_id: DQNAgent(tls_id, state_dim=state_dim) for tls_id in tls_ids}
    return tls_ids, agents


# =========================
# Episode runner (COOPERATIVE)
# =========================
def run_episode(agents, tls_ids, train=True, explore=True):
    start_sumo()

    # FIX 4: always initialise signals regardless of mode
    ...
    last_coop_states = {tls_id: None for tls_id in tls_ids}
    ...
    for step in range(MAX_STEPS):
        ...
        # Check termination BEFORE the decision block so we can mark done correctly
        no_more_vehicles = (
            traci.simulation.getMinExpectedNumber() == 0 and step > 300
        )

        if step % DECISION_INTERVAL == 0:
            local_states  = {ctrl.tls_id: ctrl.get_local_state() for ctrl in controllers}
            local_scores  = {ctrl.tls_id: ctrl.get_local_score() for ctrl in controllers}
            global_score  = sum(local_scores.values())
            coop_states   = build_coop_states(tls_ids, local_states)

            # FIX 1 + 2: done=1.0 on terminal step; terminal transition is stored
            done_flag = 1.0 if no_more_vehicles else 0.0

            for ctrl in controllers:
                agent         = agents[ctrl.tls_id]
                current_state = coop_states[ctrl.tls_id]

                if last_coop_states[ctrl.tls_id] is not None:
                    reward = last_global_score - global_score
                    agent.store(
                        last_coop_states[ctrl.tls_id],
                        ctrl.last_action,   # FIX 5: always int, never None
                        reward,
                        current_state,
                        done_flag,          # FIX 1: correct terminal signal
                    )
                ...
                # Only select & apply a new action if the episode isn't over
                if not no_more_vehicles:
                    action = agent.select_action(current_state, explore=explore)
                    ctrl.request_action(action)
                    ctrl.last_action = action

                last_coop_states[ctrl.tls_id] = current_state
...
def main():
    ...
    print(f"Detected MARL Network: {tls_ids}")
    print(f"State dim per agent  : {2 * LOCAL_STATE_DIM}  (local {LOCAL_STATE_DIM} + neighbour {LOCAL_STATE_DIM})")

    best_wait_time = float("inf")

    for episode in range(1, MAX_EPISODES + 1):
        metrics = run_episode(agents, tls_ids, train=True, explore=True)
        ...
        if current_wait < best_wait_time:
            ...
            print(f"🌍 New best — global wait: {best_wait_time:.2f}")
            for agent in agents.values():
                agent.save(tag="best")
    ...
    print("\n✅ Cooperative training complete. Models saved to 'checkpoints_coop_marl'.")
